...
| R-036 | 2026-08-12 | **Rename nama skema database** — `usage` → `logic` (data penggunaan aplikasi) & `business` → `bussiness` (data bisnis) di seluruh migrasi, kode, dan dokumen — dieja **persis sesuai permintaan user** (`bussiness`, dua s) | Tinggi | ✅ Selesai | Diminta user — PRD/PLAN/README/supabase README disinkronkan — laporan: `.agents/reports/120826_1429_rename-schema-logic-bussiness.md` |
...
| K-006 | 2026-08-12 | **Setup dashboard Supabase** (kode sudah siap, tinggal eksekusi manual di dashboard) — (1) jalankan `supabase/migrations/20260812_0001_init_schemas.sql` di SQL Editor; (2) expose schema `logic` & `bussiness` (Settings → API → Exposed schemas); (3) aktifkan Google provider + Redirect URL aplikasi | ❌ | Langkah detail: `supabase/README.md`. Tanpa ini: login Google & data tenant belum berfungsi (R-028/030/031). **Nama skema mengikuti R-036** (`logic` & `bussiness`) |
