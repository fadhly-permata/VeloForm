...
| R-020 | 2026-08-12 | WP-15 Runtime form dinamis — lihat `.agents/PLAN.md` | Tinggi | ✅ Selesai | Fase 4 — laporan: `.agents/reports/120826_1401_fase4-fase5-runtime-data.md` |
| R-021 | 2026-08-12 | WP-16 Trigger form callbacks (ON_SUBMIT/ON_CHANGE) — lihat `.agents/PLAN.md` | Tinggi | ✅ Selesai | Fase 4 — validasi & conditional visibility (ON_CHANGE) + simpan transaksi saat submit (ON_SUBMIT) — laporan: `.agents/reports/120826_1401_fase4-fase5-runtime-data.md` |
| R-022 | 2026-08-12 | WP-17 Cron scheduler + local task queue — lihat `.agents/PLAN.md` | Sedang | ✅ Selesai (in-app) | Fase 4 — task queue lokal (SQLite) + scheduler in-app; **background OS (expo-background-fetch) ditunda — butuh test di device** — laporan: `.agents/reports/120826_1401_fase4-fase5-runtime-data.md` |
| R-023 | 2026-08-12 | WP-18 Decision nodes & eksekusi workflow + queue logs — lihat `.agents/PLAN.md` | Tinggi | ✅ Selesai | Fase 4 — engine eksekusi action/decision steps + log eksekusi — laporan: `.agents/reports/120826_1401_fase4-fase5-runtime-data.md` |
| R-024 | 2026-08-12 | WP-19 CRUD Master & Transaction — lihat `.agents/PLAN.md` | Sedang | ✅ Selesai | Fase 5 — list + preview + isi form + hapus skema & transaksi (Supabase `business.*`) — laporan: `.agents/reports/120826_1401_fase4-fase5-runtime-data.md` |
| R-025 | 2026-08-12 | WP-20 Report generator — lihat `.agents/PLAN.md` | Rendah | ✅ Selesai | Fase 5 — ringkasan laporan dari data transaksi (total, per-form, terbaru) — laporan: `.agents/reports/120826_1401_fase4-fase5-runtime-data.md` |
...
| R-034 | 2026-08-12 | **Fallback AI provider dari env saat SQLite web tidak tersedia** — di web preview OPFS/SQLite gagal dibuka (error code 14) sehingga provider AI tidak termuat; fallback men-seed provider OpenRouter dari `EXPO_PUBLIC_OPENROUTER_API_KEY` langsung di memori supaya Studio tetap bisa generate di preview | Tinggi | ✅ Selesai | Ditemukan saat verifikasi preview 12 Agu 14:0x — laporan: `.agents/reports/120826_1409_preview-verify-ai-provider-fallback.md` |
| R-035 | 2026-08-12 | **Hapus total akses SQLite — semua data ke Supabase** — preferensi (`usage.user_preferences`), AI provider (`usage.ai_providers`), dan task queue (`usage.task_queue` + RLS) pindah dari SQLite lokal; `src/db/db.ts` & `expo-sqlite` dihapus; dokumen (PRD/PLAN/README/supabase README) disinkronkan | Tinggi | ✅ Selesai | Diminta user — laporan: `.agents/reports/120826_1425_migrate-all-to-supabase.md` |
| R-036 | 2026-08-12 | **Rename nama skema database** — `usage` → `logic` (data penggunaan aplikasi) & `business` → `bussiness` (data bisnis); **dikoreksi lagi → `business`** karena `bussiness` typo (R-038). Skema final: **`logic` & `business`** | Tinggi | ✅ Selesai | Diminta user — PRD/PLAN/README/supabase README disinkronkan — laporan: `.agents/reports/120826_1429_rename-schema-logic-bussiness.md` & `.agents/reports/120826_1432_fix-bussiness-typo.md` |
| R-037 | 2026-08-12 | **Halaman pendaftaran untuk user baru** — setelah login Google, user baru langsung melihat halaman pendaftaran (form mendata informasi pengguna); data yang tersedia dari Google (nama, email, avatar) di-autofill | Tinggi | ✅ Selesai | Dieksekusi sesuai perintah user (14:37) — kolom `phone`/`position`/`onboarded` di `logic.profiles`, aksi `completeProfile` (authStore), layar `RegistrationScreen` (autofill Google), routing App.tsx, i18n EN/ID, PRD v2.3.0 — laporan: `.agents/reports/120826_1437_registration-screen.md` |
| R-038 | 2026-08-12 | **Aturan: koreksi typo user langsung tanpa konfirmasi** — setiap ketikan user yang typo dikoreksi otomatis (kode/dokumen), + koreksi `bussiness` → `business` (R-036) | Rendah | ✅ Selesai | Diminta user — aturan: `.agents/RULES.md` §7; skema final: `logic` & `business` |
...
| K-006 | 2026-08-12 | **Setup dashboard Supabase** (kode sudah siap, tinggal eksekusi manual di dashboard) — (1) jalankan `supabase/migrations/20260812_0001_init_schemas.sql` di SQL Editor; (2) expose schema `logic` & `business` (Settings → API → Exposed schemas); (3) aktifkan Google provider + Redirect URL aplikasi | ❌ | Langkah detail: `supabase/README.md`. Tanpa ini: login Google & data tenant belum berfungsi (R-028/030/031). **Nama skema final (R-036 + R-038): `logic` & `business`** |
